...
TITLESEARCH = Path(__file__).resolve().parents[1]
COMPANIES_PATH = TITLESEARCH / "data" / "companies.json"
OUT_DIR = TITLESEARCH / "data" / "raw"
...
def build_queries(company: str) -> list[str]:
    ...
    ML-variant filtering (machine/deep/reinforcement learning engineer) is handled
    post-retrieval by is_le_hit(), so query-level exclusions are intentionally omitted.
    Brave's API does not reliably honour -"phrase" operators, and aggressive exclusions
    also suppress valid results on pages that legitimately co-mention machine learning.
    """
    return [
        f'"{company}" "learning engineer"',
        f'site:linkedin.com/in "learning engineer" "{company}"',
    ]
