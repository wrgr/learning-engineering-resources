...
export function GraphCanvas({ nodes, links, selectedNodeId, onSelect }) {
  ...
  useEffect(() => {
    ...
    const linkSelection = svg
      ...
      .join("line")
      .attr("stroke-width", (edge) => (edge.type === "contains" ? 1 : edge.type === "resource" ? 0.9 : 1.45))
      .attr("stroke", (edge) => {
        if (selectedNodeId) {
          if (edge.target?.id === selectedNodeId || edge.target === selectedNodeId) return "#0f766e";
          if (edge.source?.id === selectedNodeId || edge.source === selectedNodeId) return "#ea580c";
          return "#cbd5e1";
        }
        if (edge.type === "resource") return "#0f766e";
        if (edge.type === "expands_to") return "#ca8a04";
        if (edge.type === "prereq") return "#0f766e";
        return "#94a3b8";
      })
      .attr("stroke-opacity", (edge) => {
        if (!selectedNodeId) return 0.5;
        if (edge.source?.id === selectedNodeId || edge.source === selectedNodeId) return 0.85;
        if (edge.target?.id === selectedNodeId || edge.target === selectedNodeId) return 0.85;
        return 0.12;
      });
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          ...
          .id((node) => node.id)
          .distance((edge) => (edge.type === "contains" ? 92 : edge.type === "resource" ? 84 : 54))
      )
