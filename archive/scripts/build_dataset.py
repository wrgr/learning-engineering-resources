...
def _paper_enrichment_stub(seed_papers: List[Dict], hop_papers: List[Dict]) -> Dict:
    """Summary stats when skipping network enrichment; keys align with `enrich_papers_with_openalex` return."""
    papers = seed_papers + hop_papers
    missing = sum(1 for p in papers if not (p.get("abstract") or "").strip())
    proxy_n = sum(1 for p in papers if bool(p.get("abstract_is_proxy")))
    without_source = sum(
        1 for p in papers if bool(p.get("abstract_is_proxy")) or not (p.get("abstract_source") or "").strip()
    )
    return {
        "papers_total": len(papers),
        "papers_with_openalex_match": 0,
        "abstracts_filled": 0,
        "openalex_title_lookups": 0,
        "openalex_resolved_by_title": 0,
        "papers_missing_abstract": missing,
        "papers_with_proxy_description": proxy_n,
        "papers_without_source_abstract": without_source,
        "crossref_doi_candidates": 0,
        "crossref_doi_fetched": 0,
        "crossref_abstracts_filled": 0,
        "papers_missing_abstract_after_crossref": missing,
        "arxiv_candidates": 0,
        "arxiv_fetched": 0,
        "arxiv_abstracts_filled": 0,
        "papers_missing_abstract_after_arxiv": missing,
        "url_abstract_urls_checked": 0,
        "url_abstract_urls_fetched": 0,
        "url_abstracts_filled": 0,
        "url_pdf_urls_checked": 0,
        "url_pdf_urls_fetched": 0,
        "url_pdf_abstracts_filled": 0,
...
def _skip_paper_enrichment_requested(args: argparse.Namespace) -> bool:
    """True when CLI or `SKIP_PAPER_ENRICHMENT` requests a fast build (no OpenAlex/CrossRef/URL abstract passes)."""
    if getattr(args, "skip_paper_enrichment", False):
        return True
    return os.getenv("SKIP_PAPER_ENRICHMENT", "").strip().lower() in ("1", "true", "yes")


def parse_args() -> argparse.Namespace:
    """Parse CLI flags for `build_dataset.py`."""
    p = argparse.ArgumentParser(description="Build website data artifacts from the Learning Engineering corpus.")
    p.add_argument(
...
def _fallback_topic_code(topic_by_code: Dict[str, Topic]) -> str:
    """Prefer T00 (field overview); otherwise first topic code in sorted order."""
    if "T00" in topic_by_code:
        return "T00"
    return sorted(topic_by_code.keys())[0]


def _synthetic_resource_id(row: Dict, stable_index: int) -> str:
    """Stable OpenAlex-style id so graph nodes and dedupe work when registry omits resource_id."""
    payload = json.dumps(
        {
...
def build_resources(topic_by_code: Dict[str, Topic]) -> Tuple[Dict, List[Dict]]:
    ...
    for idx, row in enumerate(rows):
        ...
        if not rid:
            rid = _synthetic_resource_id(r, idx)
            r["resource_id"] = rid
            log.append(f"synthetic resource_id {rid} for {(r.get('name') or 'Untitled')[:72]!r}")
        normalized.append(r)
    ...
    for row in rows:
        raw_primary = (row.get("primary_topic") or "").strip()
        if raw_primary in topic_by_code:
            primary = raw_primary
        else:
            primary = _fallback_topic_code(topic_by_code)
            log.append(
                f"unknown primary_topic {raw_primary!r} for [{row.get('resource_id')}]; "
                f"using {primary} for {(row.get('name') or '')[:56]!r}"
            )
...
def build_programs(non_paper_rows: List[Dict]) -> Dict:
    """Map every non-paper resource row into programs_summary (unknown content_type → category `other`)."""
    category_map = {
        "AP": "academic_papers",
        "PC": "academic",
        "PP": "people",
    ...
    for row in non_paper_rows:
        ct = (row.get("content_type") or "").strip() or "GL"
        programs.append(
            {
                "name": row.get("title", ""),
                "category": category_map.get(ct, "other"),
...
def main() -> None:
    ...
    if _skip_paper_enrichment_requested(args):
