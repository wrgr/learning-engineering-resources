...
def _openalex_budget_or_rate_limit(body_text: str) -> bool:
    """True when OpenAlex JSON error indicates quota, budget, or rate limiting."""
    ...
    try:
        ...
        return False
    msg = f"{payload.get('message', '')} {payload.get('error', '')}".lower()
    if "insufficient budget" in msg or "rate limit" in msg:
        return True
...
def _retry_after_seconds_from_openalex_body(body_text: str, attempt: int) -> float:
    """Prefer API-provided retryAfter (seconds), capped to avoid multi-hour sleeps in scripts."""
    cap = 120.0
    if body_text and body_text.strip().startswith("{"):
        try:
            payload = json.loads(body_text)
            ra = payload.get("retryAfter")
    ...
    for attempt in range(OPENALEX_MAX_RETRIES + 1):
        try:
            ...
            try:
                ...
                err_body = ""
            budget_hit = _openalex_budget_or_rate_limit(err_body)
            retriable = exc.code in {429, 500, 502, 503, 504} or (
                budget_hit and exc.code in {400, 401, 403, 404, 429}
            )
            ...
            if exc.headers:
                ...
                if raw:
                    try:
                        ...
                        retry_after = 0.0
            if retry_after <= 0 and budget_hit:
                retry_after = _retry_after_seconds_from_openalex_body(err_body, attempt)
