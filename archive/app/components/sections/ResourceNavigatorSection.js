...
export function ResourceNavigatorSection({
  ...
  resourceQuery,
  setResourceQuery,
  includeCommunityResources,
  setIncludeCommunityResources,
}) {
    ...
    <section id="resources" className="panel">
      <div className="panel-head">
        <div>
          ...
          <p className="caption">
            People, programs, conferences, tools, grey literature, and more — sourced from the ICICLE and programs/people
            registries (plus supplemental rows), grouped by resource type below.
            ${!includeCommunityResources
              ? html` <span className="caption-hint">SEED-tier registry rows are hidden until you enable community resources.</span>`
              : html` <span className="caption-hint">Including SEED-tier rows and topic, concept, and paper nodes from the corpus graph.</span>`}
          </p>
        </div>
      </div>

      <div className="resource-controls">
        <label className="resource-community-toggle">
          <input
            type="checkbox"
            checked=${includeCommunityResources}
            onChange=${(e) => setIncludeCommunityResources(e.target.checked)}
          />
          <span>Include community resources</span>
        </label>
        <input
