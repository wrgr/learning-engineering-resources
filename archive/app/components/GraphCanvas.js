...
function getNodeRadius(node) {
  ...
  if (node.type === "paper" && (node.hop || 0) >= 2) return 3.5;
...
function getNodeColor(node) {
  ...
  if (node.type === "paper" && (node.hop || 0) >= 2) return NODE_COLORS.paper_merged;
...
export function GraphCanvas({ nodes, links, selectedNodeId, onSelect, search, typeFilter, resetRef }) {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        "collision",
        d3.forceCollide().radius((n) =>
          n.type === "paper" && (n.hop || 0) >= 1 ? 7 : 10
        )
      )
